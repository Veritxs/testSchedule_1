...
/** Splits a day column when classes overlap in time. */
function layoutDay(entries: ScheduleEntry[]) {
  ...
  return sorted.map((entry) => ({
    entry,
    columnIndex: columns.findIndex((items) => items.includes(entry)),
...
export function WeekGrid({
  ...
  const byDay = useMemo(
    () =>
      weekDays.map((date) => ({
        date,
        items: layoutDay(entries.filter((entry) => entry.date === date)),
      })),
    [entries, weekDays],
  )
            ...
            {items.map(({ entry, columnIndex, columnCount }) => {
              ...
              return (
                ...
                >
                  ...
                  {height >= 52 && (
                    <span>
                      {entry.sessionNumber ? `S${entry.sessionNumber} · ` : ''}
                      {entry.startTime}
                    </span>
                  )}
                </button>
